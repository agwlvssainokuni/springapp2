...
import cherry.fundamental.mail.MailData;
import cherry.fundamental.mail.TemplateStore;
import cherry.gallery.db.gen.query.QMailTemplate;
import cherry.gallery.db.gen.query.QMailTemplateRcpt;
...
public class TemplateStoreImpl implements TemplateStore {
	...
	public MailData getTemplate(String templateName) {
		...
		Tuple templ = querya.select(mt.id, mt.fromAddr, mt.replyToAddr, mt.subject, mt.body).fetchOne();

		SQLQuery<?> queryb = queryFactory.from(mtr);
		queryb.where(mtr.templateId.eq(templ.get(mt.id)));
		List<Tuple> templAddr = queryb.select(mtr.rcptType, mtr.rcptAddr).fetch();
	...
	public void putTemplate(String templateName, MailData mailData) {
		...
		querya.where(mt.templateName.eq(templateName));
		querya.forUpdate();
		Long id = querya.select(mt.id).fetchOne();
		if (id == null) {
