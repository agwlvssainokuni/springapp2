...
public class RSASignatureSupportTest {
	...
	private RSASignatureSupport createCrypto() throws Exception {
		...
		crypto.setPublicKeyResource(new ByteArrayResource(key.getPublic().getEncoded()));
		crypto.setPrivateKeyResource(new ByteArrayResource(key.getPrivate().getEncoded()));
		crypto.initialize();
		return crypto;
	}
