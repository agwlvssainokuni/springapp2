...
import static cherry.foundation.type.EnumCodeUtil.getCodeList;
...
public class EnumCodeUtilTest {
	...
	@Test
	public void testGetCodeListForString() {
		List<FlagCode> list = getCodeList(FlagCode.class.getName());
		assertThat(list.size(), is(2));
		assertThat(list.get(0), is(FlagCode.FALSE));
		assertThat(list.get(1), is(FlagCode.TRUE));
	}
	...
	@Test
	public void testGetCodeMapForString() {
		Map<Integer, FlagCode> map = getCodeMap(FlagCode.class.getName());
		assertThat(map.size(), is(2));
		assertThat(map.get(11), is(FlagCode.FALSE));
		assertThat(map.get(22), is(FlagCode.TRUE));
	}
	...
	@Test
	public void testGetCodeListForString_NotEnum() {
		try {
			getCodeList(NotEnumCode.class.getName());
			fail("Exception must be thrown");
		} catch (IllegalArgumentException ex) {
			// OK
		}
	...
	@Test
	public void testGetCodeMapForString_NotEnum() {
		try {
			getCodeMap(NotEnumCode.class.getName());
			fail("Exception must be thrown");
		} catch (IllegalArgumentException ex) {
			// OK
		}
	...
	@Test
	public void testGetCodeListForString_NOTEXIST() {
		try {
			getCodeList("NOTEXIST");
			fail("Exception must be thrown");
		} catch (IllegalArgumentException ex) {
			// OK
		}
	...
	@Test
	public void testGetCodeMapForString_NOTEXIST() {
		try {
			getCodeMap("NOTEXIST");
			fail("Exception must be thrown");
		} catch (IllegalArgumentException ex) {
