 ...
 * <li>N営業日後算出機能: 所定のカレンダーに基づき、起点日から指定の営業日数後の営業日を算出する。営業日数には起点日を含む。即ち、営業日数に「1」を指定すると、(当日が営業日ならば)起点日と同じ日付が得られる。</li>
 * </ul>
 */
public interface WorkdayManager {
	 ...
	 * @return 営業日数。
	 */
	<T extends ICodeType<String>> int getNumberOfWorkday(T code, LocalDate to);
	 ...
	 * @return 営業日数。
	 */
	<T extends ICodeType<String>> int getNumberOfWorkday(T code, LocalDate from, LocalDate to);
	 ...
	 * @return <code>numberOfWorkday</code>営業日後の営業日。
	 */
	<T extends ICodeType<String>> LocalDate getNextWorkday(T code, int numberOfWorkday);
	 ...
	 * @return <code>numberOfWorkday</code>営業日後の営業日。
	 */
	<T extends ICodeType<String>> LocalDate getNextWorkday(T code, LocalDate from, int numberOfWorkday);
